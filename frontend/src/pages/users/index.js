    ...
    },
    {
        ...
        render: (text,record) => {
            return <div className='flex gap-1'>
                {record.isVerified ? (<button className='primary-outline-btn' onClick={()=>updateStatus(record,false)}>Suspend</button>) : (<button className='primary-outline-btn' onClick={()=>updateStatus(record,true)}>Activate</button>)}
            </div>
        }
    }
  ]
